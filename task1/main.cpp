...
double linear_approx_tri(const Cell &c, function<double (double, double)> f, double x, double y)
{
	ElementArray<Node> nodes = c.getNodes();
	double res = 0.0;
	for(unsigned i = 0; i < 3; i++){
...
double integrate_over_triangle(const Cell &c, function<double (double, double)> f)
{
	double res = 0.0;
...
auto diff_square(const Cell &c, function<double (double, double)> f) {
    auto d = [c, f](double x, double y) -> double {
        return pow(f(x, y) - linear_approx_tri(c, f, x, y), 2);
    };
    return d;
}
...
void main_diff_node_funcs(Mesh &m)
{
	...
	for(Mesh::iteratorCell icell = m.BeginCell(); icell != m.EndCell(); icell++){
		Cell c = icell->getAsCell();
        auto diff_func = diff_square(c, g); // (g - g_approx)^2
		normL2 += integrate_over_triangle(c, diff_func);
        ...
        for(unsigned i = 0; i < 3; i++){
            ...
            double x = coords[0];
            double y = coords[1];
            normC += max(normC, abs(g(x, y) - linear_approx_tri(c, g, x, y)));
        }
